...
from terms_mapper import TermsMapper
...
def embed_query(encoder: BGEM3FlagModel, query: str) -> np.ndarray:
    out = encoder.encode_queries([query])
...
def read_chunk_text(meta: Dict[str, Any], kb_dir: str, old_base: Optional[str]) -> str:
    ...
    try:
        with open(src, "r", encoding="utf-8") as f:
            full = f.read()
        s, e = meta.get("start") or meta.get("start_char"), meta.get("end") or meta.get("end_char")
...
def main():
    ...
    cfg = load_config(r"C:\practicum\am-ml\Task4\rag_config.yaml")
    kb_dir    = cfg["kb_dir"]
    ...
    answer = call_llm(base_url, api_key, model, messages, temperature, max_tokens)

    try:
        mapper = TermsMapper(TERMS_MAP_PATH)
        answer = mapper.transform(answer, mode="annotate")
    except Exception as e:
        print(f"[terms_map] skip: {e}", file=sys.stderr)
